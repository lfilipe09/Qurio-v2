import TextContentMock01 from 'components/TextContent/mocks/lideranca/03/mock01'
import TextContentMock02 from 'components/TextContent/mocks/lideranca/03/mock02'
import TextContentMock03 from 'components/TextContent/mocks/lideranca/03/mock03'

export default [
  {
    img: '/img/packs/lideranca/03/lideranca-parte03-thumb01.png',
    title: '5 lições sobre o poder da vulnerabilidade',
    format: 'Vídeo',
    content: TextContentMock01.content,
    publicationDate: '2022-04-24T15:27:03.044Z',
    reference: 'Brown, Brené. TED. Palestra: O poder da Vulnerabilidade',
    link: 'https://docs.google.com/presentation/d/1xvdv4XQ1YPHOFz5TkBvJdSfdzIroWmpZoc93REoHd4Q/edit?usp=sharing',
    imgDesktop: [
      '/img/packs/lideranca/03/lideranca-parte03-content-01.1-desktop.jpg',
      '/img/packs/lideranca/03/lideranca-parte03-content-01.2-desktop.jpg',
      '/img/packs/lideranca/03/lideranca-parte03-content-01.3-desktop.jpg',
      '/img/packs/lideranca/03/lideranca-parte03-content-01.4-desktop.jpg',
      '/img/packs/lideranca/03/lideranca-parte03-content-01.5-desktop.jpg'
    ],
    imgMobile: [
      '/img/packs/lideranca/03/lideranca-parte03-content-01.1-mobile.jpg',
      '/img/packs/lideranca/03/lideranca-parte03-content-01.2-mobile.jpg',
      '/img/packs/lideranca/03/lideranca-parte03-content-01.3-mobile.jpg',
      '/img/packs/lideranca/03/lideranca-parte03-content-01.4-mobile.jpg',
      '/img/packs/lideranca/03/lideranca-parte03-content-01.5-mobile.jpg'
    ],
    quiz: false,
    buttonUrl:
      'https://www.ted.com/talks/brene_brown_the_power_of_vulnerability?language=pt'
  },
  {
    img: '/img/packs/lideranca/03/lideranca-parte03-thumb02.png',
    title: 'Liderança inspiradora: as novas regras da gestão de pessoas',
    format: 'Tabela Comparativa',
    content: TextContentMock02.content,
    publicationDate: '2022-04-24T15:27:03.044Z',
    reference:
      'HSM Management#151. Abril, 2022. Sua organização está na mira da Great Resignation; Página 12',
    link: 'https://docs.google.com/presentation/d/1og5zGLKzuqxYo9-vLkHC1tDB_Aw_WZYWsn0qeCM4wMc/edit?usp=sharing',
    imgDesktop: [
      '/img/packs/lideranca/03/lideranca-parte03-content-03.1-desktop.jpg',
      '/img/packs/lideranca/03/lideranca-parte03-content-03.2-desktop.jpg'
    ],
    imgMobile: [
      '/img/packs/lideranca/03/lideranca-parte03-content-03.1-mobile.jpg',
      '/img/packs/lideranca/03/lideranca-parte03-content-03.2-mobile.jpg'
    ],
    quiz: false,
    buttonUrl:
      'https://www.revistahsm.com.br/post/sua-organizacao-esta-na-mira-da-great-resignation?gclid=CjwKCAjwquWVBhBrEiwAt1Kmwn57JDguoWiO7feZQjnBVNQ44j2s1ZTpzeOh--XbDgWontl3XdZw4xoC2T0QAvD_BwE'
  },
  {
    img: '/img/packs/lideranca/03/lideranca-parte03-thumb03.png',
    title: 'A importância do repertório cultural',
    format: 'Infográfico',
    content: TextContentMock03.content,
    publicationDate: '2022-04-24T15:27:03.044Z',
    reference:
      'HSM Management#147. 2021. Seu repertório é elástico - Página 56',
    link: 'https://docs.google.com/presentation/d/16yMh7frfjto4xdRMr4opLc9kJK5NtRMBss0g7HBYNOY/edit?usp=sharing',
    imgDesktop: [
      '/img/packs/lideranca/03/lideranca-parte03-content-02.1-desktop.jpg',
      '/img/packs/lideranca/03/lideranca-parte03-content-02.2-desktop.jpg',
      '/img/packs/lideranca/03/lideranca-parte03-content-02.3-desktop.jpg'
    ],
    imgMobile: [
      '/img/packs/lideranca/03/lideranca-parte03-content-02.1-mobile.jpg',
      '/img/packs/lideranca/03/lideranca-parte03-content-02.2-mobile.jpg',
      '/img/packs/lideranca/03/lideranca-parte03-content-02.3-mobile.jpg'
    ],
    quizPoint: true,
    buttonUrl: 'https://www.revistahsm.com.br/post/seu-repertorio-e-elastico'
  }
]
